...
void main() {
  group('calculateLettersPerSpaceRatio', () {
    test('should calculate correct ratio for strings with letters and spaces', () {
      ...
      expect(calculateLettersPerSpaceRatio('This is a test'), closeTo(3.67, 0.01));
    ...
    test('should return infinity for strings with no spaces', () {
      ...
      expect(calculateLettersPerSpaceRatio('Programming'), equals(double.infinity));
      expect(calculateLettersPerSpaceRatio('Flutter'), equals(double.infinity));
      expect(calculateLettersPerSpaceRatio(''), equals(double.infinity)); // edge case
    });
    ...
    test('should handle multiple consecutive spaces correctly', () {
      ...
      expect(calculateLettersPerSpaceRatio(' Hello World '), closeTo(3.33, 0.01));
    });

    test('should handle special formatting characters', () {
      // Tab is not a space
      expect(calculateLettersPerSpaceRatio('Hello\tWorld'), equals(double.infinity));
      
      // Newline is not a space
      expect(calculateLettersPerSpaceRatio('Hello\nWorld'), equals(double.infinity));
  ...
  group('shouldAutomaticallyBeConsideredAValidString', () {
    group('basic validation', () {
      ...
      test('Returns true for multiple words with only letters and spaces', () {
        expect(shouldAutomaticallyBeConsideredAValidString('Home Page'), isTrue);
        expect(
    ...
    group('space ratio validation', () {
      test('should reject strings with low letters-per-space ratio', () {
        ...
        expect(shouldAutomaticallyBeConsideredAValidString('h e l l o'), isFalse);
        expect(shouldAutomaticallyBeConsideredAValidString('W o r l d'), isFalse);
        expect(shouldAutomaticallyBeConsideredAValidString('a  b  c'), isFalse);
        expect(shouldAutomaticallyBeConsideredAValidString('ab cd ef'), isFalse); // ratio = 3.0
      ...
      test('should handle strings with exactly threshold ratio', () {
        ...
        expect(shouldAutomaticallyBeConsideredAValidString('abcd efgh'), isFalse);
        
        // "Abcd efgh" has ratio of 8.0 and matches the pattern
        expect(shouldAutomaticallyBeConsideredAValidString('Abcd efgh'), isTrue);
      });
      ...
      test('should reject strings with only spaces or no letters', () {
        ...
        expect(shouldAutomaticallyBeConsideredAValidString(r'!@# $%^'), isFalse);
      });
    });
    ...
    group('length and empty validation', () {
      ...
      test('should reject strings longer than 150 characters', () {
        final longString = 'A' + 'a' * 150; // 151 characters
        expect(shouldAutomaticallyBeConsideredAValidString(longString), isFalse);
        
        // Exactly 150 characters should be accepted (if it matches other criteria)
        final maxLengthString = 'A' + 'a' * 149; // 150 characters
        expect(shouldAutomaticallyBeConsideredAValidString(maxLengthString), isTrue);
      });
    });

    group('consecutive spaces validation', () {
      test('should reject strings with 4 or more consecutive spaces', () {
        expect(shouldAutomaticallyBeConsideredAValidString('Hello    World'), isFalse);
        expect(shouldAutomaticallyBeConsideredAValidString('Test     Case'), isFalse);
        expect(shouldAutomaticallyBeConsideredAValidString('One    Two    Three'), isFalse);
      });

      test('should handle strings with 3 or fewer consecutive spaces based on pattern', () {
        // Test individual cases to understand the pattern better
        // Multi-word strings that match the complex pattern may be accepted
        expect(shouldAutomaticallyBeConsideredAValidString('Test Case'), isTrue);
    ...
    group('integration tests', () {
      test('should properly validate real-world examples', () {
        ...
        expect(shouldAutomaticallyBeConsideredAValidString('S a r a h'), isFalse);
        ...
        expect(shouldAutomaticallyBeConsideredAValidString(veryLongString), isFalse);
      });
      ...
      test('should handle edge cases correctly', () {
        ...
        // Mixed content
        expect(shouldAutomaticallyBeConsideredAValidString('Hello123'), isFalse);
        expect(shouldAutomaticallyBeConsideredAValidString('Test!'), isFalse);
        
        // Edge case patterns - single letters don't match the pattern
        expect(shouldAutomaticallyBeConsideredAValidString('I'), isFalse); // Single letter
        expect(shouldAutomaticallyBeConsideredAValidString('A'), isFalse); // Single letter
      });
    });
  });
}